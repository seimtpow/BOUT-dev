...
public:
  auto static get_offsets(int n_compact, int n_taxi, int n_cross, int nz) {
  ...
  int static get_star_pattern(int stencil_size, int n2d, int n3d, int nz) {
    ...
    // either
    // stencil_size * (n2d + n3d) + 2 * n3d
    // or
    // stencil_size * (n2d + n3d) + (count3D - stencil_size)*n3d
...
int SNESSolver::init() {
  ...
  if (matrix_free) {
    ...
    if (use_coloring) {
      ...
      auto const star_pattern =
          ColoringStencil::get_star_pattern(xyoffsets.size(), n2d, n3d, mesh->LocalNz);

      output_info.write("Star pattern: {} non-zero entries\n", star_pattern);

      // Non-zero elements on this processor
      std::vector<PetscInt> d_nnz(localN, star_pattern);
      // Non-zero elements on neighboring processor
      std::vector<PetscInt> o_nnz(localN, 0);

      //Simple brute force loop over all the elements in the mesh
      //that could be near a boundary and check if the stencil is out
      //outside of domain/on another rank and update non-zero counts
      auto max_extent = std::max({n_square, n_taxi, n_cross});
      for (int x = mesh->xstart; x <= mesh->xend; ++x) {
        //only need to iterate over start and end of range
        //Not pretty but skip the loop variable forward
        if (x == mesh->xstart + max_extent + 1 && x < mesh->ystart - max_extent) {
          x = mesh->xend - max_extent;
        }
        for (int y = mesh->ystart; y <= mesh->yend; ++y) {
          if (y == mesh->ystart + max_extent + 1 && y < mesh->ystart - max_extent) {
            y = mesh->yend - max_extent;
          }
          int ind0 = ROUND(index(x, y, 0));
          //2D fields
          for (int i = 0; i < n2d; ++i) {
            int row = ind0 + i;
            for (auto const& [x_off, y_off, _] : xyoffsets) {
              int xi = x + x_off;
              int yi = y + y_off;
              if (xi < mesh->xstart) {
                d_nnz[row] -= n2d;
              ...
              } else if (xi >= mesh->xend) {
                d_nnz[row] -= n2d;
                if (!mesh->lastX()) {
                  o_nnz[row] += n2d;
                }
              } else if (yi < mesh->ystart) {
                d_nnz[row] -= n2d;
                if (!mesh->hasBndryLowerY()) {
                  o_nnz[row] += n2d;
                }
              } else if (yi >= mesh->yend) {
                d_nnz[row] -= n2d;
      ...
      for (int x = mesh->xstart; x <= mesh->xend; x++) {
        for (int y = mesh->ystart; y <= mesh->yend; y++) {
          ...
          for (int i = 0; i < n2d; i++) {
            ...
            for (const auto& [x_off, y_off, _] : xyoffsets) {
              int xi = x + x_off;
              int yi = y + y_off;
